...
int UpdateCompressTaskResultInfo(char* task_id, int iTaskStatus, int iTaskResult, int iErrorCode, int iCompressBeginTime, int iCompressEndTime, int iYSHFileSize, char* pcDestUrl, DBOper* pDbOper)
{
    ...
    if (1 == iTaskStatus)
    {
        iRet = update_compress_task_to_db_proc(task_id, 5, 0, NULL);
        DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "compress_task_set_task_result_info: update_compress_task_to_db_proc :task_id=%s, iRet=%d\r\n", task_id, iRet);
    }
    else if (2 == iTaskStatus || 3 == iTaskStatus)
    {
        if (1 == iTaskResult)
        {
            iRet = update_compress_task_to_db_proc(task_id, 6, 0, pcDestUrl);
            DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "compress_task_set_task_result_info: update_compress_task_to_db_proc :task_id=%s, iRet=%d\r\n", task_id, iRet);
        }
        else if (2 == iTaskResult)
        {
            iRet = update_compress_task_to_db_proc(task_id, 7, iErrorCode, NULL);
            DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "compress_task_set_task_result_info: update_compress_task_to_db_proc :task_id=%s, iRet=%d\r\n", task_id, iRet);
        }
    }
