...
int platform_srv_proc_thread_assign(platform_info_t* pPlatformInfo)
{
    ...
    {
        if (g_CompressTaskDBOper.Connect(g_CompressTaskDBConn, (char*)"") < 0)
        {
            g_CompressTaskDBOperConnectStatus = 0;
        }
        else
        {
            g_CompressTaskDBOperConnectStatus = 1;
        }
    }
...
int get_compress_task_from_webservice_thread_execute(void* p)
{
    ...
    if (1 == run->iCompressTaskStatus) /* 准备获取 */
    {
        ...
        DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "get_compress_task_from_webservice_thread_execute() get_compress_task_from_db_proc:Begin---\r\n");
        iRet = get_compress_task_from_db_proc(run->platform_ip, iBeginTime, iEndTime, run->pPlatform_Srv_dboper);
        DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "get_compress_task_from_webservice_thread_execute() get_compress_task_from_db_proc:End---:iRet=%d\r\n", iRet);
