...
extern DBOper g_CompressTaskDBOper;                                                    /* 压缩任务数据库连接标识 */
extern int g_CompressTaskDBOperConnectStatus;                                          /* 压缩任务数据库连接状态 */
...
#if DECS("通过数据库接口获取压缩任务处理")
/*****************************************************************************
 函 数 名  : get_compress_task_from_db_proc
 功能描述  : 通过数据库接口获取压缩任务
...
*****************************************************************************/
int get_compress_task_from_db_proc(char* platform_ip, int iTaskBeginTime, int iTaskEndTime, DBOper* ptDBoper)
{
    int iRet = 0;

    if (NULL == platform_ip || platform_ip[0] == '\0')
    {
        DEBUG_TRACE(MODULE_COMMON, LOG_ERROR,  "get_compress_task_from_db_proc() exit---: platform_ip Error \r\n");
        return -1;
    }
    ...
    /* 3、通过数据库获取需要压缩的任务信息 */
    iRet = get_compress_task_by_db(platform_ip, ptDBoper);
    ...
    {
        SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_NORMAL, "从上级平台获取压缩任务信息, 没有获取到数据库记录");
    }
    else
    {
...
*****************************************************************************/
int get_compress_task_by_db(char* platform_ip, DBOper* ptDBoper)
{
    int iRet = 0;
    ...
    if (NULL == platform_ip || platform_ip[0] == '\0')
    {
        DEBUG_TRACE(MODULE_COMMON, LOG_ERROR,  "get_compress_task_by_db() exit---: platform_ip Error \r\n");
        return -1;
    }

    if (1 != g_CompressTaskDBOperConnectStatus)
    {
        SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_ERROR, "从上级平台获取压缩任务信息失败, 数据库没有连接成功");
        return FILE_COMPRESS_DB_CONNECT_ERROR;
    }

    SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_NORMAL, "通过查询数据库获取压缩任务信息:platform_ip=%s", platform_ip);

    strSQL.clear();
    strSQL = "select * from compressfileinfo where istatus = 0 AND idatatype = 1";

    record_count = g_CompressTaskDBOper.DB_Select(strSQL.c_str(), 1);

    if (record_count < 0)
    {
        DEBUG_TRACE(MODULE_COMMON, LOG_ERROR, "get_compress_task_by_db() DB Oper Error:strSQL=%s, record_count=%d \r\n", strSQL.c_str(), record_count);
        DEBUG_TRACE(MODULE_COMMON, LOG_ERROR, "get_compress_task_by_db() ErrorMsg=%s\r\n", g_CompressTaskDBOper.GetLastDbErrorMsg());
        SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_ERROR, "从上级平台获取压缩任务信息失败，数据库操作失败, strSQL=%s, iRet=%d", strSQL.c_str(), record_count);
        return FILE_COMPRESS_DB_QUERY_ERROR;
    }
    else if (record_count == 0)
    {
        SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_ERROR, "从上级平台获取压缩任务信息失败，没有获取到数据库数据, strSQL=%s, iRet=%d", strSQL.c_str(), record_count);
        DEBUG_TRACE(MODULE_COMMON, LOG_WARN, "get_compress_task_by_db() exit---: No Record Count \r\n");
        return 0;
    }

    DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "get_compress_task_by_db() DB Record:strSQL=%s, record_count=%d \r\n", strSQL.c_str(), record_count);
    ...
    do
    {
        ...
        record_count++;
        DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "get_compress_task_by_db() Load GBDevice info: count=%d", record_count);
        ...
        /* 唯一标识符 */
        tmp_svalue.clear();
        g_CompressTaskDBOper.GetFieldValue("szuuid", tmp_svalue);

        if (!tmp_svalue.empty())
        {
            osip_strncpy(stYSPB.jlbh, tmp_svalue.c_str(), MAX_TSU_TASK_LEN);
            SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_NORMAL, "从上级平台获取压缩任务信息成功，获取到的任务ID=%s, 任务数=%d", stYSPB.jlbh, record_count);
        }
        else
        {
            SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_WARNING, "从上级平台获取压缩任务信息失败，获取到的任务ID为空");
            DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "get_compress_task_by_db() szuuid NULL");
            continue;
        }

        /* 压缩前路径 */
        tmp_svalue.clear();
        g_CompressTaskDBOper.GetFieldValue("szfilepath", tmp_svalue);

        if (!tmp_svalue.empty())
        {
            osip_strncpy(stYSPB.cclj, tmp_svalue.c_str(), 128);
            SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_NORMAL, "从上级平台获取压缩任务信息成功，获取到的文件路径=%s", stYSPB.cclj);
        }
        else
        {
            SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_WARNING, "从上级平台获取压缩任务信息失败，获取到的文件路径为空");
            DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "get_compress_task_by_db() szfilepath NULL");
            continue;
        }

        /* 压缩后路径 */
        tmp_svalue.clear();
        g_CompressTaskDBOper.GetFieldValue("szcompressedfilepath", tmp_svalue);

        if (!tmp_svalue.empty())
        {
            osip_strncpy(stYSPB.yshlj, tmp_svalue.c_str(), 128);
        }

        /* 类型:1代表视频文件， 2代表图片文件 */
        tmp_ivalue = 0;
        g_CompressTaskDBOper.GetFieldValue("idatatype", tmp_ivalue);
        ...
        if (0 != iRet)
        {
            DEBUG_TRACE(MODULE_COMMON, LOG_ERROR,  "get_compress_task_by_db() exit---: AddCompressTask Error:platform_ip=%s, iRet=%d \r\n", platform_ip, iRet);
            continue;
        }
    }
    while (g_CompressTaskDBOper.MoveNext() >= 0);
...
int update_compress_task_to_db_proc(char* jlbh, int istatus, int ierrorno, char* pcDestURL)
{
    int iRet = 0;
    string strUpdateSQL = "";

    char strCompressTaskStatus[32] = {0};
    char strCompressTaskErrorNo[32] = {0};

    if (NULL == jlbh)
    {
        DEBUG_TRACE(MODULE_COMMON, LOG_DEBUG, "update_compress_task_to_db_proc() exit---:  Param Error \r\n");
        return -1;
    }

    if (1 != g_CompressTaskDBOperConnectStatus)
    {
        DEBUG_TRACE(MODULE_COMMON, LOG_DEBUG, "update_compress_task_to_db_proc() exit---:  g_CompressTaskDBOperConnectStatus Error \r\n");
        return FILE_COMPRESS_DB_CONNECT_ERROR;
    }

    snprintf(strCompressTaskStatus, 32, "%d", istatus);
    snprintf(strCompressTaskErrorNo, 32, "%d", ierrorno);

    strUpdateSQL.clear();
    strUpdateSQL = "UPDATE compressfileinfo SET";

    strUpdateSQL += " istatus = ";
    ...
    if (7 == istatus)
    {
        strUpdateSQL += ",";
        strUpdateSQL += " ierrno = ";
        strUpdateSQL += strCompressTaskErrorNo;
    }
    else
    {
        strUpdateSQL += ",";
        strUpdateSQL += " ierrno = 0";
    }

    if (NULL != pcDestURL)
    {
        strUpdateSQL += ",";
        strUpdateSQL += " szcompressedfilepath = '";
    ...
    strUpdateSQL += " WHERE szuuid like '";
    strUpdateSQL += jlbh;
    strUpdateSQL += "'";

    iRet = g_CompressTaskDBOper.DB_Update(strUpdateSQL.c_str(), 1);

    if (iRet < 0)
    {
        SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_ERROR, "压缩任务结果更新到远端服务器数据库失败:记录编号=%s, 更新状态=%s, 错误码=%s, 压缩后路径=%s", jlbh, strCompressTaskStatus, strCompressTaskErrorNo, pcDestURL);
        DEBUG_TRACE(MODULE_COMMON, LOG_ERROR, "update_compress_task_to_db_proc() DB Oper Error:strUpdateSQL=%s, iRet=%d \r\n", strUpdateSQL.c_str(), iRet);
        DEBUG_TRACE(MODULE_COMMON, LOG_ERROR, "update_compress_task_to_db_proc() ErrorMsg=%s\r\n", g_CompressTaskDBOper.GetLastDbErrorMsg());
        return FILE_COMPRESS_DB_UPDATE_ERROR;
    }
    else
    {
        SystemLog(EV9000_CMS_SYSTEM_RUN_INFO, EV9000_LOG_LEVEL_NORMAL, "更新压缩任务结果到远端服务器数据库成功:记录编号=%s, 更新状态=%s, 错误码=%s, 压缩后路径=%s", jlbh, strCompressTaskStatus, strCompressTaskErrorNo, pcDestURL);
    }
