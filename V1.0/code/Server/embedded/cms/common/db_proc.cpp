...
DBOper g_CompressTaskDBOper;                                                        /* 压缩任务数据库连接标识 */
int g_CompressTaskDBOperConnectStatus = 0;                                          /* 压缩任务数据库连接状态 */

DB_Thread* g_DBThread;
char g_strDBThreadPara[4][100] = {{0}, {0}, {0}, {0}};
char g_StrCon[2][100] = {{0}, {0}};
char g_StrConLog[2][100] = {{0}, {0}};
char g_CompressTaskDBConn[2][100] = {{0}, {0}};                  /* 压缩任务的数据库连接字符串 */
...
int db_gDBOperconnect()
{
	...
	/* 压缩任务获取数据库连接 */
    iRet = g_CompressTaskDBOper.Connect(g_CompressTaskDBConn, (char*)"");

    printf("db_gDBOperconnect() g_CompressTaskDBOper Connect: iRet=%d\r\n", iRet);

    if (0 != iRet)
    {
    	g_CompressTaskDBOperConnectStatus = 0;
        return 0; /* 数据库没有连接成功，不能退出 */
    }
