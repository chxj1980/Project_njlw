...
extern char g_CompressTaskDBConn[2][100];
...
char g_CompressTaskDBAddr[MAX_128CHAR_STRING_LEN + 4] = {0};     /* 压缩任务的数据库地址 */
char g_CompressTaskDBUserName[MAX_128CHAR_STRING_LEN + 4] = {0}; /* 压缩任务的数据库用户名 */
char g_CompressTaskDBPasswd[MAX_128CHAR_STRING_LEN + 4] = {0};   /* 压缩任务的数据库密码 */
char g_CompressTaskDBName[MAX_128CHAR_STRING_LEN + 4] = {0};     /* 压缩任务的数据库名称 */
...
#define CONF_COMMPRESS_TASK_DB_ADDR ("CompressTaskDBAddr")          /* 压缩任务数据库地址 */
#define CONF_COMMPRESS_TASK_DB_USERNAME ("CompressTaskDBUserName")  /* 压缩任务数据库用户名 */
#define CONF_COMMPRESS_TASK_DB_PASSWORD ("CompressTaskDBPassword")  /* 压缩任务数据库密码 */
#define CONF_COMMPRESS_TASK_DB_NAME ("CompressTaskDBName")          /* 压缩任务数据库名称 */
...
int gbl_conf_load()
{
    ...
    memset(g_CompressTaskDBConn[0], 0, 100);
    snprintf(g_CompressTaskDBConn[0], 100, "%s;%s;%s;%s;3306", g_CompressTaskDBAddr, g_CompressTaskDBUserName, g_CompressTaskDBPasswd, g_CompressTaskDBName);
    memset(g_CompressTaskDBConn[1], 0, 100);
    ...
    printf(" gbl_conf_load() 压缩任务的数据库地址:g_CompressTaskDBAddr=%s \r\n", g_CompressTaskDBAddr);
    printf(" gbl_conf_load() 压缩任务的数据库用户名:g_CompressTaskDBUserName=%s \r\n", g_CompressTaskDBUserName);
    printf(" gbl_conf_load() 压缩任务的数据库密码:g_CompressTaskDBPasswd=%s \r\n", g_CompressTaskDBPasswd);
    printf(" gbl_conf_load() 压缩任务的数据库名称:g_CompressTaskDBName=%s \r\n", g_CompressTaskDBName);
    ...
    printf(" gbl_conf_load() g_CompressTaskDBConn[0]=%s \r\n", g_CompressTaskDBConn[0]);
    printf(" gbl_conf_load() g_CompressTaskDBConn[1]=%s \r\n", g_CompressTaskDBConn[1]);
...
int gbl_conf_reload()
{
    ...
    DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "gbl_conf_load() g_CompressTaskDBConn[0]=%s \r\n", g_CompressTaskDBConn[0]);
    DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "gbl_conf_load() g_CompressTaskDBConn[1]=%s \r\n", g_CompressTaskDBConn[1]);
...
int conf_reset(gbl_conf_t* conf, char* pname, char* pvalue, int iRefresh)
{
    ...
    else if (strcasecmp(pname, CONF_COMMPRESS_TASK_DB_ADDR) == 0) /* 压缩任务获取的数据库地址 */
    {
        if (0 != sstrcmp(g_CompressTaskDBAddr, pvalue))
        {
            memset(g_CompressTaskDBAddr, 0, MAX_128CHAR_STRING_LEN + 4);
            osip_strncpy(g_CompressTaskDBAddr, pvalue, MAX_128CHAR_STRING_LEN);

            if (iRefresh)
            {
                DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "conf_reset() g_CompressTaskDBAddr=%s \r\n", g_CompressTaskDBAddr);
            }
            else
            {
                printf(" conf_reset() g_CompressTaskDBAddr=%s \r\n", g_CompressTaskDBAddr);
            }
        }
    }
    else if (strcasecmp(pname, CONF_COMMPRESS_TASK_DB_USERNAME) == 0) /* 压缩任务获取的数据库用户名 */
    {
        if (0 != sstrcmp(g_CompressTaskDBUserName, pvalue))
        {
            memset(g_CompressTaskDBUserName, 0, MAX_128CHAR_STRING_LEN + 4);
            osip_strncpy(g_CompressTaskDBUserName, pvalue, MAX_128CHAR_STRING_LEN);

            if (iRefresh)
            {
                DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "conf_reset() g_CompressTaskDBUserName=%s \r\n", g_CompressTaskDBUserName);
            }
            else
            {
                printf(" conf_reset() g_CompressTaskDBUserName=%s \r\n", g_CompressTaskDBUserName);
            }
        }
    }
    else if (strcasecmp(pname, CONF_COMMPRESS_TASK_DB_PASSWORD) == 0) /* 压缩任务获取的数据库密码 */
    {
        if (0 != sstrcmp(g_CompressTaskDBPasswd, pvalue))
        {
            memset(g_CompressTaskDBPasswd, 0, MAX_128CHAR_STRING_LEN + 4);
            osip_strncpy(g_CompressTaskDBPasswd, pvalue, MAX_128CHAR_STRING_LEN);

            if (iRefresh)
            {
                DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "conf_reset() g_CompressTaskDBPasswd=%s \r\n", g_CompressTaskDBPasswd);
            }
            else
            {
                printf(" conf_reset() g_CompressTaskDBPasswd=%s \r\n", g_CompressTaskDBPasswd);
            }
        }
    }
    else if (strcasecmp(pname, CONF_COMMPRESS_TASK_DB_NAME) == 0) /* 压缩任务获取的数据名称 */
    {
        if (0 != sstrcmp(g_CompressTaskDBName, pvalue))
        {
            memset(g_CompressTaskDBName, 0, MAX_128CHAR_STRING_LEN + 4);
            osip_strncpy(g_CompressTaskDBName, pvalue, MAX_128CHAR_STRING_LEN);

            if (iRefresh)
            {
                DEBUG_TRACE(MODULE_COMMON, LOG_INFO, "conf_reset() g_CompressTaskDBName=%s \r\n", g_CompressTaskDBName);
            }
            else
            {
                printf(" conf_reset() g_CompressTaskDBName=%s \r\n", g_CompressTaskDBName);
            }
        }
    }
    else
...
void show_system_gbl_param(int sock)
{
    ...
    snprintf(rbuf, 256, "\r压缩任务获取的数据库地址               :CompressTaskGetInterval=%s \r\n", g_CompressTaskDBAddr);
    ...
    snprintf(rbuf, 256, "\r压缩任务获取的数据库用户名             :CompressTaskDBUserName=%s \r\n", g_CompressTaskDBUserName);
    ...
    snprintf(rbuf, 256, "\r压缩任务获取的数据库密码               :CompressTaskDBPasswd=%s \r\n", g_CompressTaskDBPasswd);
    ...
    snprintf(rbuf, 256, "\r压缩任务获取的数据名称                 :CompressTaskDBName=%s \r\n", g_CompressTaskDBName);
